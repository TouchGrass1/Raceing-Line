...
class ProcessTrack:
    ...
    def colour_track_boundaries(image, height, width):
            ...
            order = ProcessTrack.flood_fill(img_arr, start_blue, start_green)
            return order
    ...
    def flood_fill(img_arr, start_blue, start_green):
        ...
        for i, my_set in enumerate(sets):
            while my_set:
                ...
                for nx, ny in ProcessTrack.candidate_neighbors(node, size=5):
    ...
    def find_order(img_arr, start_blue, start_green, colours):
        ...
        for i in range(2):
            ...
            while run:
                ...
                for size in [3, 5, 11]: # Loop through progressively larger neighborhood sizes
                    neighbors = ProcessTrack.candidate_neighbors(node, size=size)
                    ...
                    if closest is not None:
                        if size == 11:
                            ProcessTrack.join_gap(order[i][-1], closest)
        ...
        ValidateTrack.check_return_to_start(order, start_blue, start_green)
        return order
...
class OrderOfOperations:
    def __init__(self, track_name, dark_mode=False):
    ...
    def run(self):
        self.track_image, self.height, self.width = LoadTrack.import_tracks(self.track_name)
        if self.track_image is None:
            print("file not found")
        else:
            self.order = LoadTrack.load_order(self.track_name)
            if self.order is None:
                if ValidateTrack.valid_track(self.track_image):
                    self.order = ProcessTrack.colour_track_boundaries(self.track_image, self.height, self.width)
                    self.order = np.array(self.order, dtype=object)
                    LoadTrack.save_order(self.order, self.track_name)
            ProcessTrack.change_bg_colour(self.track_image, self.dark_mode)
